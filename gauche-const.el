...
(defconst gauche-const-procedure-keyword-list
  '( 
    ;; define methods
...
(defvar gauche-defined-procedure-keyword-regexp
  (regexp-opt gauche-const-procedure-keyword-list))

(defconst *gauche-scheme-r5rs-info*
...
(defconst *gauche-scheme-srfi-info*
  [
   ;; SRFI 0
...
(defconst *gauche-documented-exports*
  '(
    (E2BIG integer)
    (EACCES integer)
    (EADDRINUSE integer)
    ...
    (^ (lambda (formals body \.\.\.)))
    (^c (lambda (body \.\.\.)))
    (cut (lambda (expr-or-slot expr-or-slot2 \.\.\.)))
    (cute (lambda (expr-or-slot expr-or-slot2 \.\.\.)))
    (case-lambda (lambda (clause \.\.\.)))
    (set! (lambda (symbol expression)))
    (set! (lambda (\(proc\ arg\ \.\.\.\) expression)))
    (set!-values (lambda (\(var\ \.\.\.\) expr)))
    (setter (lambda (proc)))
    (has-setter\? (lambda (proc)))
    (getter-with-setter (lambda (get set)))
    (push! (lambda (place item)))
    (pop! (lambda (place)))
    (inc! (lambda (place :optional delta)))
    (dec! (lambda (place :optional delta)))
    (update! (lambda (place proc)))
    (if (lambda (test consequent alternative)))
    (if (lambda (test consequent)))
    (cond (lambda (clause1 clause2 \.\.\.)))
    (case (lambda (key clause1 clause2 \.\.\.)))
    (and (lambda (test \.\.\.)))
    (or (lambda (test \.\.\.)))
    (when (lambda (test body \.\.\.)))
    (unless (lambda (test body \.\.\.)))
    (let (lambda (\(\(var\ expr\)\ \.\.\.\) body \.\.\.)))
    (let* (lambda (\(\(var\ expr\)\ \.\.\.\) body \.\.\.)))
    (letrec (lambda (\(\(var\ expr\)\ \.\.\.\) body \.\.\.)))
    (let1 (lambda (var expr body \.\.\.)))
    (if-let1 (lambda (var expr then)))
    (if-let1 (lambda (var expr then else)))
    (rlet1 (lambda (var expr body \.\.\.)))
    (and-let* (lambda (\(binding\ \.\.\.\) body \.\.\.)))
    (fluid-let (lambda (\(\(var\ val\)\ \.\.\.\) body \.\.\.)))
    (receive (lambda (formals expression body \.\.\.)))
    (rec (lambda (var expr)))
    (rec (lambda (\(name\ \.\ vars\) expr \.\.\.)))
    (begin (lambda (form \.\.\.)))
    (begin0 (lambda (exp0 exp1 \.\.\.)))
    (do (lambda (\(\(variable\ init\ \[step\]\)\ \.\.\.\) \(test\ expr\ \.\.\.\) body \.\.\.)))
    (let (lambda (name bindings body \.\.\.)))
    (dotimes (lambda (\(variable\ limit\ \[result\]\) body \.\.\.)))
    (dolist (lambda (\(variable\ lexpr\ \[result\]\) body \.\.\.)))
    (while (lambda (expr body \.\.\.)))
    (while (lambda (expr \'=>\' var body \.\.\.)))
    (while (lambda (expr guard \'=>\' var body \.\.\.)))
    (until (lambda (expr body \.\.\.)))
    (until (lambda (expr guard \'=>\' var body \.\.\.)))
    (quasiquote (lambda (template)))
    (unquote (lambda (datum)))
    (unquote-splicing (lambda (datum)))
    (define (lambda (variable expression)))
    (define (lambda (\(variable\ \.\ formals\) body \.\.\.)))
    (define-values (lambda (\(var\ \.\.\.\) expr)))
    (define-constant (lambda (variable expression)))
    (define-constant (lambda (\(variable\ \.\ formals\) body \.\.\.)))
    (define-in-module (lambda (module variable expression)))
    (define-in-module (lambda (module \(variable\ \.\ formals\) body \.\.\.)))
    (define-module (lambda (name body \.\.\.)))
    (select-module (lambda (name)))
    (with-module (lambda (name body \.\.\.)))
    (current-module (lambda nil))
    (export (lambda (symbol \.\.\.)))
    (export-all (lambda nil))
    (import (lambda (import-spec \.\.\.)))
    (use (lambda (name :key (only) (except) (rename) (prefix))))
    (extend (lambda (module-name \.\.\.)))
    (module\? (lambda (obj)))
    ...
    (define-syntax (lambda (name transformer-spec)))
    (let-syntax (lambda (\(\(name\ transformer-spec\)\ \.\.\.\) body)))
    (letrec-syntax (lambda (\(\(name\ transformer-spec\)\ \.\.\.\) body)))
    (syntax-rules (lambda (literals clause1 clause2 \.\.\.)))
    (define-macro (lambda (name procedure)))
    (define-macro (lambda (\(name\ \.\ formals\) body \.\.\.)))
    (macroexpand (lambda (form)))
    (macroexpand-1 (lambda (form)))
    (%macroexpand (lambda (form)))
    (%macroexpand-1 (lambda (form)))
    (syntax-error (lambda (msg arg \.\.\.)))
    (syntax-errorf (lambda (fmt arg \.\.\.)))
    ...
    (rxmatch-let (lambda (match-expr \(var\ \.\.\.\) form \.\.\.)))
    (rxmatch-if (lambda (match-expr \(var\ \.\.\.\) then-form else-form)))
    (rxmatch-cond (lambda (clause \.\.\.)))
    (rxmatch-case (lambda (string-expr clause \.\.\.)))
    ...
    (let-optionals* (lambda (restargs \(var-spec\ \.\.\.\) body \.\.\.)))
    (let-optionals* (lambda (restargs \(var-spec\ \.\.\.\ \.\ restvar\) body \.\.\.)))
    (get-optional (lambda (restargs default)))
    (let-keywords (lambda (restarg \(var-spec\ \.\.\.\) body \.\.\.)))
    (let-keywords (lambda (restarg \(var-spec\ \.\.\.\ \.\ restvar\) body \.\.\.)))
    (let-keywords* (lambda (restarg \(var-spec\ \.\.\.\) body \.\.\.)))
    (let-keywords* (lambda (restarg \(var-spec\ \.\.\.\ \.\ restvar\) body \.\.\.)))
    (arity (lambda (proc)))
    ...
    (call/cc (lambda (proc)))
    (let/cc (lambda (var body \.\.\.)))
    (dynamic-wind (lambda (before thunk after)))
    (values (lambda (obj \.\.\.)))
    (call-with-values (lambda (producer consumer)))
    (values-ref (lambda (mv-expr k)))
    (lazy (lambda (expression)))
    (delay (lambda (expression)))
    (force (lambda (promise)))
    (promise\? (lambda (obj)))
    ...
    (guard (lambda (\(var\ clause\ \.\.\.\) body \.\.\.)))
    (unwind-protect (lambda (body cleanup)))
    (with-error-handler (lambda (handler thunk)))
    (current-exception-handler (lambda nil))
    (with-exception-handler (lambda (handler thunk)))
    (define-condition-type (lambda (name supertype predicate field-spec \.\.\.)))
    (condition-type\? (lambda (obj)))
    ...
    (extract-condition (lambda (condition condition-type)))
    (condition (lambda (type-field-binding \.\.\.)))
    (eval (lambda (expr env)))
    ...
    (add-load-path (lambda (path :optional)))
    ...
    (require (lambda (feature)))
    (provide (lambda (feature)))
    (provided\? (lambda (feature)))
    (autoload (lambda (file/module item \.\.\.)))
    ...
    (with-signal-handlers (lambda (\(handler-clause\ \.\.\.\) thunk)))
    ...
    (define-class (lambda (name supers \(slot-spec\ \.\.\.\) option \.\.\.)))
    ...
    (define-generic (lambda (name :key (class))))
    (define-method (lambda (name specs body)))
    ...
    (apropos (lambda (pattern :optional module)))
    (info (lambda (symbol)))
    ...
    (parameterize (lambda (\(\(param\ value\)\ \.\.\.\) body \.\.\.)))
    (let-args (lambda (args)))
    (parse-options (lambda (args \(option-clause\ \.\.\.\))))
    (make-option-parser (lambda (\(option-clause\ \.\.\.\))))
    (reset (lambda (expr \.\.\.)))
    (shift (lambda (var expr \.\.\.)))
    (call/pc (lambda (proc)))
    ...
    (define-record-type (lambda (type-spec ctor-spec pred-spec field-spec \.\.\.)))
    (record\? (lambda (obj)))
    ...
    (test* (lambda (name expected expr :optional compare)))
    ...
    (time (lambda (expr expr2 \.\.\.)))
    (with-time-counter (lambda (counter expr \.\.\.)))
    ...
    (cond-expand (lambda (\(feature-requirement\ command-or-definition\ \.\.\.\) \.\.\.)))
    (xcons (lambda (cd ca)))
    ...
    (let (lambda (name)))
    (let (lambda nil))
    (let-values (lambda (\(\(vars\ expr\)\ \.\.\.\) body \.\.\.)))
    (let*-values (lambda (\(\(vars\ expr\)\ \.\.\.\) body \.\.\.)))
    ...
    (let-string-start+end (lambda (\(start\ end\ \[rest\]\) proc-exp s-exp args-exp body \.\.\.)))
    ...
    (do-ec (lambda (qualifier \.\.\. body)))
    (list-ec (lambda (qualifier \.\.\. body)))
    (append-ec (lambda (qualifier \.\.\. body)))
    (string-ec (lambda (qualifier \.\.\. body)))
    (string-append-ec (lambda (qualifier \.\.\. body)))
    (vector-ec (lambda (qualifier \.\.\. body)))
    (vector-of-length-ec (lambda (k qualifier \.\.\. body)))
    (sum-ec (lambda (qualifier \.\.\. body)))
    (product-ec (lambda (qualifier \.\.\. body)))
    (min-ec (lambda (qualifier \.\.\. body)))
    (max-ec (lambda (qualifier \.\.\. body)))
    (any\?-ec (lambda (qualifier \.\.\. test)))
    (every\?-ec (lambda (qualifier \.\.\. test)))
    (first-ec (lambda (default qualifier \.\.\. body)))
    (last-ec (lambda (default qualifier \.\.\. body)))
    (fold-ec (lambda (seed qualifier \.\.\. expr proc)))
    (fold3-ec (lambda (seed qualifier \.\.\. expr init proc)))
    ...
    (require-extension (lambda (clause \.\.\.)))
    ...
    (xml-token-kind (lambda (token)))
    (xml-token-head (lambda (token)))
    ...
    (ssax:make-pi-parser (lambda (my-pi-handlers)))
    (ssax:make-elem-parser (lambda (my-new-level-seed my-finish-element my-char-data-handler my-pi-handlers)))
    (ssax:make-parser (lambda (user-handler-tag user-handler-proc \\\.\\\.\\\.)))
    ...
    (match (lambda (expr clause \.\.\.)))
    (match-lambda (lambda (clause \.\.\.)))
    (match-lambda* (lambda (clause \.\.\.)))
    (match-let (lambda (\(\(pat\ expr\)\ \.\.\.\) body-expr \.\.\.)))
    (match-let (lambda (name \(\(pat\ expr\)\ \.\.\.\) body-expr \.\.\.)))
    (match-let* (lambda (\(\(pat\ expr\)\ \.\.\.\) body-expr \.\.\.)))
    (match-letrec (lambda (\(\(pat\ expr\)\ \.\.\.\) body-expr \.\.\.)))
    (match-let1 (lambda (pat expr body-expr \.\.\.)))
    (match-define (lambda (pat expr)))
    ...
    (stream\? (lambda (obj)))
    (stream-cons (lambda (object stream)))
    (stream-null\? (lambda (obj)))
    (stream-pair\? (lambda (obj)))
    (stream-car (lambda (s)))
    (stream-cdr (lambda (s)))
    (stream-delay (lambda (expr)))
    (stream (lambda (obj \.\.\.)))
